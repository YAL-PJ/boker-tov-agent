...
- [x] **2.1** Define `state.json` schema: `{session_active, session_started_at, flow_position, frozen, current_task, timer, last_session_file}`.
- [x] **2.2** n8n: a Code/Execute node reads `state.json` at the start of every message and routes: no active session → "new session" path; active → normal chat path.
- [x] **2.3** New-session path: LLM-generated warm welcome (for now without the check-in — just the welcome), mark session active.
- [x] **2.4** "End day" detection (user writes סיים יום / נגמר להיום): archive `today.md` → `archive/YYYY-MM-DD-sessionN.md`, update `last_session_file`, reset state, fresh `today.md`. Nightly cron becomes the fallback for unclosed sessions only.
...
- [x] **3.1** Flow engine: `flow_position` in `state.json` tracks which question is next. n8n injects into the system prompt: the flow spec + current position + instruction "ask question N, respond as coach to the answer."
